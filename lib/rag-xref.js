...
// chunks, extract useful code symbols, and scan the already
// loaded index for definitions / imports / call sites.
//
...
function collectSymbolsFromChunks(chunks, limit = MAX_SYMBOLS) {
    ...
    for (const chunk of chunks) {
        ...
        for (const symbol of extractUsefulSymbols(chunk.content ?? "")) {
            ...
            if (ordered.length >= limit || addedFromChunk >= MAX_SYMBOLS_PER_CHUNK) {
                break;
            }
        }
...
function collectXrefCandidates(selected, allChunks) {
    ...
    let seedChunks = topImplementationChunks(selected, 3);
    ...
    for (let hop = 0; hop < MAX_HOPS && seedChunks.length > 0; hop += 1) {
        ...
        const matches = findMatchesForSymbols(
            allChunks,
            symbols,
            selectedKeys,
            seedFiles
        ...
        for (const match of matches) {
            ...
            if (!existing) {
                ...
                const sameCategoryEarlierHop =
                    record.category === existing.category && record.hop < existing.hop;

                if (betterCategory || sameCategoryEarlierHop) {
                    candidateMap.set(key, record);
                }
            }
            ...
            if (
                match.impl &&
                (match.rawCategory === "definition" || match.rawCategory === "call") &&
                nextSeeds.length < MAX_HOP_SEED_CHUNKS
            ) {
...
function xrefInternalScore(match, definedInSelected = new Set()) {
...
function selectionScore(item, definedInSelected) {
    ...
    return Math.min(790, 520 + xrefInternalScore(item, definedInSelected) + sim);
}
...
function greedySelect(pool, topK) {
    const selected = [];
    const keys = new Set();
    ...
    for (const item of pool) {
        ...
        const key = chunkIdentity(item.chunk);

        if (keys.has(key)) {
            continue;
        }

        const fileKey = normalizePath(item.chunk.filePath);
...
export function expandWithCrossReferences(selected, allChunks, topK) {
    ...
    const seedChunks = topImplementationChunks(selected, 3);
    ...
    for (const item of pool) {
        item.score = selectionScore(item, definedInSelected);
    }

    pool.sort(comparePoolItems);

    const merged = greedySelect(pool, topK);
