...
//       ↓
//   (Project answering only) expand with cross-file
//     definition / import / call-site evidence
//       ↓
...
// Project answering only. Chat and Computer omit numCtx and
// keep Ollama's current/default context.
export const RAG_NUM_CTX = 8192;
...
async function retrieveForAskInternal(question) {
    ...
    const diversified = diversifyMatches(tied, RAG_TOP_K);

    return expandWithCrossReferences(diversified, scored, RAG_TOP_K);
}


/**
 * RAG retrieval only: score, near-tie source preference, diversify,
 * then lightweight cross-reference expansion.
...
/**
 * Chat options used by askProject. Exported so tests can
 * confirm Project requests 8192 without inspecting source.
 *
 * @returns {{
 *   model: string,
 *   temperature: number,
 ...
 * }}
 */
export function getAskProjectChatOptions() {
    return {
        model: CHAT_MODEL,
        temperature: 0.2,
        numPredict: 1000,
