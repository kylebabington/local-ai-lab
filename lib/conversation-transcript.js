...
async function persistUnlocked() {
    await atomicWriteJson(transcriptPath(), { messages });
}
...
export function getTranscript() {
    return enqueueMutation(async () => {
        ...
        return { messages: messages.map((item) => ({ ...item })) };
    });
}


export function appendMessages(rawMessages) {
    return enqueueMutation(async () => {
        await initializeTranscriptUnlocked();
        ...
        for (const raw of rawMessages) {
            ...
            if (existingIds.has(cleaned.id)) {
                // Idempotent retry — do not duplicate.
                continue;
            }
        ...
        if (appended.length > 0) {
            // Validate size against the candidate before swapping memory.
...
export function patchMessage(id, rawPatch) {
    return enqueueMutation(async () => {
        await initializeTranscriptUnlocked();
