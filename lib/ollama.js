...
// Non-streaming /api/chat does not send response headers until
// generation finishes. Project RAG at 8192 on CPU can exceed
// Node fetch's default 300s headersTimeout ("fetch failed").
const OLLAMA_POST_TIMEOUT_MS = 30 * 60 * 1000;
...
// ---------------------------------------------------------
// CHAT OPTIONS / NETWORK ERRORS
// ---------------------------------------------------------

/**
 * Build the Ollama /api/chat `options` object.
 *
 * `num_ctx` is included only when numCtx is a positive integer.
 * Callers that omit it keep Ollama's current/default context.
 *
 * @param {{ temperature: number, numPredict: number, numCtx?: number }} params
 * @returns {{ temperature: number, num_predict: number, num_ctx?: number }}
 */
...
/**
 * Turn a low-level fetch/network failure into a concise,
 * operation-specific message. Stack traces stay off the
 * message; callers may attach the original error as `cause`.
 *
 * @param {string} operationMessage
 * @param {unknown} error
 * @returns {string}
 */
export function formatOllamaUnreachableError(operationMessage, error) {
    const code =
    ...
    const hasMessage = causeMessage.length > 0;

    if (hasCode && hasMessage) {
        return `${operationMessage} ${code}: ${causeMessage}`;
    }
...
function throwOllamaUnreachable(operationMessage, error) {
    throw new Error(
        formatOllamaUnreachableError(operationMessage, error),
        { cause: error },
    );
}
...
/**
 * POST JSON to Ollama and return a fetch-like response.
 *
 * Uses node:http so long Project generations are not killed by
 * undici's 300s headersTimeout. GET /api/tags still uses fetch.
 *
 * @param {string} urlString
 * @param {object} bodyObject
 */
function postOllamaJson(urlString, bodyObject) {
    const url = new URL(urlString);
    const payload = Buffer.from(JSON.stringify(bodyObject));
    ...
    return new Promise((resolve, reject) => {
        ...
        request.setTimeout(OLLAMA_POST_TIMEOUT_MS, () => {
            request.destroy();
            const timeoutError = new Error(
                "request timed out waiting for Ollama to finish",
            );
            timeoutError.cause = {
                code: "ETIMEDOUT",
                message: "request timed out waiting for Ollama to finish",
            };
            reject(timeoutError);
        });
...
export async function createEmbeddings(texts, modelName) {
    ...
    try {
        response = await postOllamaJson(OLLAMA_EMBED_URL, {
            model: modelName,
            input: texts,
        });
    } catch (error) {
        throwOllamaUnreachable(
...
export async function chatWithOllama(messages, options) {
    ...
    try {
        response = await postOllamaJson(OLLAMA_CHAT_URL, {
            model,
            messages,
            stream: false,
            options: buildOllamaGenerationOptions({
                temperature,
                numPredict,
                numCtx,
            }),
        ...
        throwOllamaUnreachable(
            "Could not reach Ollama while generating a chat response.",
            error,
        );
    }
 ...
 * Callers that omit numCtx keep Ollama's current/default context.
 *
 * @param {{
 *   messages: object[],
 *   tools: object[],
...
export async function chatWithOllamaTools(options) {
    ...
    try {
        ...
        throwOllamaUnreachable(
            "Could not reach Ollama while generating a tool-enabled chat response.",
            error,
        );
    }
